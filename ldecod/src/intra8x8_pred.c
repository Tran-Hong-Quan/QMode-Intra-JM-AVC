...
static inline int intra8x8_Q_Pred(Macroblock* currMB,    //!< current macroblock
    ColorPlane pl,         //!< current image plane
    int ioff,              //!< pixel offset X within MB
    int joff)              //!< pixel offset Y within MB
{
    ...
    if ((!block_available_up) || (!block_available_left) || (!block_available_up_left))
        printf("warning: Intra_8x8_Q_Pred prediction mode not allowed at mb %d\n", (int)currSlice->current_mb_nr);
    ...
    memcpy((*mb_pred++) + ioff, PredArray[2], 8 * sizeof(imgpel));
    memcpy((*mb_pred++) + ioff, PredArray[3], 8 * sizeof(imgpel));
    memcpy((*mb_pred++) + ioff, PredArray[4], 8 * sizeof(imgpel));
    memcpy((*mb_pred++) + ioff, PredArray[5], 8 * sizeof(imgpel));
    memcpy((*mb_pred++) + ioff, PredArray[6], 8 * sizeof(imgpel));
    memcpy((*mb_pred  ) + ioff, PredArray[7], 8 * sizeof(imgpel));
...
{  
  ...
  case Q_PRED:  
    return (intra8x8_Q_Pred(currMB, pl, ioff, joff));
  default:
